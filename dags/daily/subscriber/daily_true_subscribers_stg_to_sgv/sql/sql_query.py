...
INSERT INTO {{ grading_schema }}.{{ temp_product_table }} 
SELECT product_id, partition_date, CURRENT_DATE() AS processed_date
...
WHERE product_id IN (SELECT product_id FROM {{ grading_schema }}.temp_product_ids);
"""
...
USING (
    WITH ranked_subscribers AS (
        ...
        FROM {{ grading_schema }}.{{ staging_table }} 
        WHERE UPPER(product_status) = 'INACTIVE' AND partition_date = '{{ partition_date }}'
    ), 
