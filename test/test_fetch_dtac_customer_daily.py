...
def test_dag_loaded(dagbag):
    ...
    assert dag is not None, f"DAG 'fetch_dtac_customer_daily' failed to load. Import errors: {dagbag.import_errors}"
    assert dagbag.import_errors == {}, f"DAG import errors: {dagbag.import_errors}"
    assert len(dag.tasks) == 4
    logger.info(f"DAG 'fetch_dtac_customer_daily' loaded successfully with tasks: {dag.task_ids}")
